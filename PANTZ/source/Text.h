...
// Whether or not a string ends with an end line character
bool Text::endline (const string& input) {
    // If the string is empty, it does not end with an end line character
    if (input.size() == 0) {return false;}
    // Otherwise check to see if it is a line feed (\n) or carriage feed (\r)
    size_t I = input.size() - 1;
    return ((input[I] == 10) || (input[I] == 13));
}
